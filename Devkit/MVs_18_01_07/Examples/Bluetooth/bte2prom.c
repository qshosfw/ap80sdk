...
uint32_t FlshGetAliveAddrWriteOffset(uint32_t StartOffset)
{
    uint32_t i;
    uint8_t Tmp[4];
    uint32_t Step = ALIVE_ADDR_REC_SIZE;//9 uint8_t for alive addr (3 majic, 6 addr)
...
bool FlshGetAliveAddr(uint8_t *Addr)
{
    uint32_t i;
    uint8_t Tmp[4];
    uint32_t StartOffset,Step = ALIVE_ADDR_REC_SIZE;//9 uint8_t for alive addr (3 majic, 6 addr)

    //ASH_RW_INIT(&SpiCodeFlashInfo);
...
bool FlshUpdateBtLatestDevAddr(uint8_t* BtAddr)
{
    ...
    //FLASH_RW_INIT(&SpiCodeFlashInfo);
...
bool FlshGetBtPairingInfoOffsetCallBk(uint32_t* StartOffset, uint32_t* MaxSize)
{
   //FLASH_RW_INIT(&SpiCodeFlashInfo);
...
uint32_t FlshGetPairingInfoWriteOffset(uint32_t StartOffset, uint32_t Step)
{
    uint32_t i;
    uint8_t Tmp[4];

    for(i = StartOffset ; i <= StartOffset + MAX_BT_PAIRING_DATA_SIZE - Step ; i += Step)
    {
        SpiFlashRead(i,Tmp,3);
        if((Tmp[0] == 'O') && (Tmp[1] == '1') && (Tmp[2] == '8'))
        {
    ...
    return (StartOffset + MAX_BT_PAIRING_DATA_SIZE - (MAX_BT_PAIRING_DATA_SIZE % Step) - Step); 
}
...
bool FlshUpDateBtPairingInfoCallBk(uint32_t NvRamOffset, uint32_t Step/*full record block step size*/, uint32_t NvRamDataSize/**/)
{
    ...
    while(NvRamDataSize)
    {
        if(NvRamDataSize > 4)t = 4;
        else t = NvRamDataSize;
...
uint32_t CheckValidatedBTUDIF(uint8_t* UniCode16/*BE*/, uint32_t Len)
{
    ...
    for(i = 0 ; i < Len - 2 ; i += 2)
    {
        if(UniCode16[i] != 0x00)continue;
        if(UniCode16[i+1] != '@')continue;
        
        //check next sig: #
        if(i < Len - 4)
        {
            if(UniCode16[i+2] != 0x00)continue;
            if(UniCode16[i+3] != '#')continue;
            //found "@#"
            Pos = i + 4;
            break;
        }
    ...
    //check next sig: "#"
    if(Len - i < 4)return BTUDIF_INFO_ERROR;
    for(i = Pos ; i < Len ; i += 2)
    {
        if(UniCode16[i] != 0x00)continue;
        if(UniCode16[i+1] != '@')continue;
