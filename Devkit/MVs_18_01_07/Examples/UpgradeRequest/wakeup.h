...
/**
 * @brief  Put the system into sleep mode.
 * @param  clk_src: clk source setting in sleep, 
 *			clk_div: clk division setting in sleep
 * @return None.
 * @Note   This function will disabel systick timer(for in OS system, the systick will wakeup system)
 * 		     if the clk_src is used DPLL and no division, the power comsuption in sleep is about 11mA,
 		     if the clk_src is used RC	and is divided by 256, the power comsuption will be about 2.5mA in sleep.
 */
void SysSleep(uint32_t clk_src, uint32_t clk_div);

/**
 * @brief 	System wakeup flow.
 * @param  None.
 * @return None.
 * @Note   This function wil enable systick timer(for in OS system, the systick will wakeup system)
 *		     The Clock setting will be restored by settings when before going to sleep.
 */
void SysWakeup(void);
