...
const uint8_t EqSetName[EQ_STYLE_CNT][16] =
{
    { "CLASSICAL" },
    { "VOCAL BOOSTER" },
    { "FLAT" },
...
void help(void)
{
    ...
    DBG("    [E] ............... EQ SWITCH\n");
    DBG("    [P] ............... SELECT PREVIOUS EQ STYLE\n");
    DBG("    [Q] ............... SELECT NEXT EQ STYLE\n");
    DBG("    [q] ............... QUIT\n");
    DBG("\n");
}

//uint8_t AAC_DECODER_RAM[40*1024] __attribute__((at(0x20010000-0xA000)));
            ...
            while(FileOpenByNum(&FileHandle, NULL, FileIndex++))
            {
                ...
                else if((FileHandle.ShortName[8] == 'F' && FileHandle.ShortName[9] == 'L' && FileHandle.ShortName[10] == 'A'))
                {
                    DecoderType = FLAC_DECODER;
                        ...
                        (FileHandle.ShortName[8] == 'M' && FileHandle.ShortName[9] == 'P' && FileHandle.ShortName[10] == '4') ||
                        (FileHandle.ShortName[8] == 'M' && FileHandle.ShortName[9] == '4' && FileHandle.ShortName[10] == 'A') ||
                        (FileHandle.ShortName[8] == 'M' && FileHandle.ShortName[9] == 'O' && FileHandle.ShortName[10] == 'V'))
                {
                ...
                //if(RT_SUCCESS == audio_decoder_initialize((uint8_t*)AAC_DECODER_RAM, (void*)FileHandle, IO_TYPE_FILE, DecoderType))
                {
                    ...
                    //Configure EQ
                    EqDisable();
                    EqStyleSelect(audio_decoder->song_info->sampling_rate, CurrEqIndex);
                    if(EqFlag)
                    {
                        EqEnable();
                    }

                    //audio_decoder_seek(130000);
                    ...
                    while(RT_YES == audio_decoder_can_continue())
                    {
                        if(0 != (KeyValue = GetKeyValue()))
                        {
                            ...
                            case 'E' :
                                EqFlag = !EqFlag;
                                DBG("[KEY__INFO]: EQ %s %s\n", EqSetName[CurrEqIndex], EqFlag ? "ON" : "OFF");
                                EqFlag ? EqEnable() : EqDisable();
                                BreakFlag = FALSE;
                                break;
                            ...
                            case 'P' :
                                if(EqFlag)
                                {
                                    CurrEqIndex = CurrEqIndex >= 1 ? CurrEqIndex - 1 : EQ_STYLE_CNT - 1;
                                    EqDisable();
                                    EqStyleSelect(audio_decoder->song_info->sampling_rate, CurrEqIndex);
                                    EqEnable();
                                    DBG("[KEY__INFO]: EQ %s\n", EqSetName[CurrEqIndex]);
                                }
                                else
                                {
                                    DBG("[KEY__INFO]: EQ disabled\n");
                                }
                                BreakFlag = FALSE;
                                break;
                            ...
                            case 'Q' :
                                if(EqFlag)
                                {
                                    CurrEqIndex = CurrEqIndex < EQ_STYLE_CNT - 1 ? CurrEqIndex + 1 : 0;
                                    EqDisable();
                                    EqStyleSelect(audio_decoder->song_info->sampling_rate, CurrEqIndex);
                                    EqEnable();
                                    DBG("[KEY__INFO]: EQ %s\n", EqSetName[CurrEqIndex]);
