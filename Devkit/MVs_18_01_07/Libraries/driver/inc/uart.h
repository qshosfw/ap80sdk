...
/**
 * IO control command for FUART and BUART 
 */
enum
{
	UART_IOCTL_BAUDRATE_GET = 1,		/**< get F/BUART baude rate,ARG2=0 */ 
	UART_IOCTL_BAUDRATE_SET,			/**< set F/BUART buade rate,ARG2=BAUDRATE,{1,200-3,000,000/bps} */ 
	UART_IOCTL_RXFIFO_TRGRDEP_GET,		/**< get F/BUART RX interrupt trigger level,FIFO depth{1-4} in bytes repensented by{0-3},ARG2=0 */ 
	UART_IOCTL_RXFIFO_TRGRDEP_SET,		/**< set F/BUART RX interrupt trigger level,FIFO depth{1-4} in bytes repensented by{0-3},ARG2={0-3}  */ 
	UART_IOCTL_RXFIFO_CLR,				/**< clear F/BUART RX interrnal FIFO forcely,ARG2=0 */ 
	UART_IOCTL_DISENRX_SET,				/**< dis/enable F/BUART RX receiption,ARG2{1,0},1 for enable,0 for disable     */ 
	UART_IOCTL_DISENTX_SET,				/**< dis/enable F/BUART TX receiption,ARG2{1,0},1 for enable,0 for disable */ 
	UART_IOCTL_RXINT_SET,				/**< dis/enable F/BUART RX interrupt,ARG2{1,0},1 for enable,0 for disable */ 
	UART_IOCTL_RX_ERR_INT_SET,			/**< dis/enable F/BUART RX error interrupt,ARG2{1,0},1 for enable,0 for disable */ 
	UART_IOCTL_TXINT_SET,				/**< dis/enable F/BUART TX interrupt,ARG2{1,0},1 for enable,0 for disable */ 
	UART_IOCTL_RXINT_CLR,				/**< clean F/BUART RX data or error interrupt status */ 
	UART_IOCTL_RX_ERR_INT_CLR,			/**< clean F/BUART RX error data or error interrupt status */ 
	UART_IOCTL_TXINT_CLR,				/**< clean F/BUART TX data interrupt status */ 
	UART_IOCTL_RXSTAT_GET,				/**< get F/BUART RX status register,5`bxxxxx,{frame_err,parity_err,overrun,dat_err,dat_int} */ 
	UART_IOCTL_TXSTAT_GET,				/**< get F/BUUART TX status register,2`bxx,{tx_busy,dat_int} */ 

	//*******************************for BT UART (BUART mode) only***************************
	BUART_IOCTL_RXFIFO_DEPTH_GET,		/**< get BUART RX FIFO depth in bytes,ARG2=0 */ 
	BUART_IOCTL_RXFIFO_DEPTH_SET,		/**< set BUART RX FIFO depth in bytes,ARG2={0-0x7FFF} */ 
	BUART_IOCTL_RXFIFO_TRGR_DEPTH_GET,	/**< get BUART RX FIFO interrupt trigger depth,ARG2=0 */ 
	BUART_IOCTL_RXFIFO_TRGR_DEPTH_SET,	/**< set BUART RX FIFO interrupt trigger depth,ARG2={0-0x7FFF} */ 
	BUART_IOCTL_TXFIFO_DEPTH_GET,		/**< get BUART TX FIFO depth in bytes,ARG2=0 */ 
	BUART_IOCTL_TXFIFO_DEPTH_SET,		/**< set BUART TX FIFO depth in bytes,ARG2={0-0x7FFF} */ 
	BUART_IOCTL_RXFIFO_SETCMD,			/**< issue set RX FIFO depth command when change FIFO depth dynamically,ARG2=0 */ 
	BUART_IOCTL_TXFIFO_SETCMD,			/**< issue set TX FIFO depth command when change FIFO depth dynamically,ARG2=0 */ 
	BUART_IOCTL_RXFIFO_DATLEN_GET,		/**< get BUART data length in RX FIFO,ARG2=0 */ 
	BUART_IOCTL_TXFIFO_FREELEN_GET,		/**< get BUART data length in TX FIFO,ARG2=0 */ 
	BUART_IOCTL_RXRTS_FLOWCTL_SET,		/**< set/unset RX RTS flow control,ARG2={0 for unset,1 for low active,2 for high active) */ 
	BUART_IOCTL_TXCTS_FLOWCTL_SET,		/**< set/unset TX CTS flow control,ARG2={0 for unset,1 for low active,2 for high active) */ 
	BUART_IOCTL_BTMODE_SET,				/**< disable/enable BUART mode or not,ARG2 = {0 for disable, 1 for enable(default)) */ 
	BUART_IOCTL_RXFIFO_CLR,				/**< clean BUART RX FIFO forcely, ARG2 = 0 */ 
	UART_IOCTL_RXFIFO_STAT_GET,			/**< get F/BUART RX FIFO status register,2`bxx,{rx_fifo_full,rx_fifo_empty} */ 
	UART_IOCTL_TXFIFO_STAT_GET,			/**< get F/BUART TX FIFO status register,2`bxx,{tx_fifo_full,tx_fifo_empty} */ 
};
 ...
 *   by DatumBits and weather or not need to parity(0 for NONE, 1 for ODD, 2 for EVEN) referred by
 *   Parity and one frame stop bits (1 or 2).
 *
 * @param	BaudRate	BaudRate can be from 1200bps to 3000000bps @96MHz core; the max BaudRate is 1500000bps @48MHz core
 * @param	DatumBits	Bit number of each frame, can be {5,6,7,8}
 * @param	Parity		parity bit, can be {0 for NONE, 1 for ODD, 2 for EVEN}
 * @param	StopBits	one frame stop bits, can be {1 or 2}
 * @return
