...
/**
 * @brief	Disable dac digital module.
...
/**
 * @brief	Enable dac digital module.
...
/**
 * @brief	Jointly set the sample rate of audio adc and dac
 ...
 * @return  None
 * @note	MCLK source of Audio ADC, Audio DAC and I2S is the same.
 */
void DacAdcSampleRateSet(uint32_t SampleRate, uint8_t Mode);

/**
 * @brief  	Get the sample rate of audio adc and dac.
 * @param	None
...
/**
 * @brief	Separately set the sample rate of audio adc and dac(audio adc = 8k and dac = 48k, mclk = 12.288MHz)
 * @param	Mode	   -> MCLK mode(USB_MODE/NORMAL_MODE/EXTERN MODE)
 * @return  None
 * @note	MCLK source of Audio ADC, Audio DAC and I2S is the same.
 */
void Adc8KDac48KSampleRateSet(uint8_t Mode);

/**
 * @brief	Separately set the sample rate of audio adc and dac(audio adc = 48k and dac = 8k, mclk = 12.288MHz)
 * @param	Mode	   -> MCLK mode(USB_MODE/NORMAL_MODE/EXTERN MODE)
 * @return  None
 * @note	MCLK source of Audio ADC, Audio DAC and I2S is the same.
 */
void Adc48KDac8KSampleRateSet(uint8_t Mode);

/**
 * @brief  	Config dac digital module. need to call this function after 'DacInit'
...
/**
 * @brief	DAC digital module volume set
 * @param   LVol	-> [0x001:-72dB, 0xFFF:0dB]
 * @param   RVol	-> [0x001:-72dB, 0xFFF:0dB]
 * @return	None
 * @note    Digital dac mute When value equals 0x0 
...
/**
 * @brief	Dac section analog and digital module mute on/off control
 * @param	LeftMuteEn	-> set to TRUE to mute the output of the Left channel DAC.
 * @param 	RightMuteEn	-> set to TRUE to mute the output of the Right channel DAC.
 * @return	None
 * @note
 ...
 * @return None
 * @Note   When AdjEn = 1 and Ppm = 0, mclk = 12MHz
*/
void DacSampRateAdjust(bool AdjEn, int32_t Ppm);

/**
 * @brief 	Set audio adc/dac clock mode
...
/**
 * @brief 	Dac analog section volume set.
 ...
 * @param	Channel			-> Indicate that which channel will strobe to dac analog section(MIXER module).
 * @return	Return TRUE if successful, or FALSE otherwise.
 ...
 * @param  VolRight	-> [0x00:21.36dB, 0x3F:-53.26dB]
 * @return NONE
 */
...
/**
 * @brief  Enable or disable mic bias
