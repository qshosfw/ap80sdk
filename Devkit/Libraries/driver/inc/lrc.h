 ...
 */
bool OpenLrcByFileName(FOLDER* CurFolder, uint8_t* FileName, uint32_t FileNameLen, uint8_t* ReadBuffer, uint32_t ReadBufSize);
 ...
 * @return 文件打开成功返回TRUE, 失败返回FALSE
 */
bool OpenLrcByFileId(FOLDER* CurFolder, uint32_t FileId, uint8_t* ReadBuffer, uint32_t ReadBufSize);

/**
 * @brief 关闭Lrc Parser
 * @param 无
 * @return 关闭文件成功返回TRUE, 否则返回FALSE
 */
